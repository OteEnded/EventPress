...
export default function DeleteOrganizerModal({ isOpen, onClose, organizerId, organizerName }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/data/organizer/delete/${organizerId}`, {
        method: "POST",
        headers: {
      ...
      // Redirect to organizer dashboard after successful deletion
      router.push("/organizer");
    } catch (error) {
      console.error("Error deleting organizer:", error);
      setError(error.message || "An error occurred while deleting the organizer.");
      setIsDeleting(false);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 relative z-10">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-6">
          คุณแน่ใจหรือไม่ว่าต้องการลบองค์กร <strong>{organizerName}</strong>?
        </p>
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 p-4 mb-6">
          ...
          <p className="text-red-700 dark:text-red-400 mt-2">
            การลบองค์กรนี้จะทำให้ข้อมูลต่อไปนี้หายไปทั้งหมด:
          </p>
          <ul className="list-disc list-inside mt-2 text-red-700 dark:text-red-400">
            <li>ข้อมูลองค์กรทั้งหมด</li>
            <li>อีเวนต์ทั้งหมดที่จัดโดยองค์กรนี้</li>
            <li>บูธทั้งหมดในอีเวนต์ขององค์กรนี้</li>
            <li>การลงทะเบียนทั้งหมดสำหรับอีเวนต์ขององค์กรนี้</li>
          </ul>
        </div>
