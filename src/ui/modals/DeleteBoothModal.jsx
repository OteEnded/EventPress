...
export default function DeleteBoothModal({ isOpen, onClose, boothId, boothName, eventIdName, organizerId }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/data/booth/delete/${boothId}`, {
        method: "POST",
        headers: {
      ...
      // Redirect back to the event page after successful deletion with the #booths hash fragment
      router.push(`/organizer/${organizerId}/event/${eventIdName}#booths`);
    } catch (error) {
      console.error("Error deleting booth:", error);
      setError(error.message || "An error occurred while deleting the booth.");
      setIsDeleting(false);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 relative z-10">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-6">
          คุณแน่ใจหรือไม่ว่าต้องการลบบูธ <strong>{boothName}</strong>?
        </p>
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 p-4 mb-6">
          ...
          <p className="text-red-700 dark:text-red-400 mt-2">
            การลบบูธนี้จะทำให้ข้อมูลต่อไปนี้หายไปทั้งหมด:
          </p>
          <ul className="list-disc list-inside mt-2 text-red-700 dark:text-red-400">
            <li>ข้อมูลบูธทั้งหมด</li>
            <li>กิจกรรมทั้งหมดในบูธนี้</li>
            <li>การลงทะเบียนทั้งหมดสำหรับบูธนี้</li>
          </ul>
        </div>
