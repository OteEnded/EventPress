...
export default function DeleteEventModal({ isOpen, onClose, eventId, eventName, organizerId }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/data/event/delete/${eventId}`, {
        method: "POST",
        headers: {
      ...
      // Redirect to organizer event list after successful deletion
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 relative z-10">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-6">
          คุณแน่ใจหรือไม่ว่าต้องการลบอีเวนต์ <strong>{eventName}</strong>?
        </p>
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 p-4 mb-6">
          ...
          <p className="text-red-700 dark:text-red-400 mt-2">
            การลบอีเวนต์นี้จะทำให้ข้อมูลต่อไปนี้หายไปทั้งหมด:
          </p>
          <ul className="list-disc list-inside mt-2 text-red-700 dark:text-red-400">
            <li>ข้อมูลอีเวนต์ทั้งหมด</li>
            <li>บูธทั้งหมดในอีเวนต์นี้</li>
            <li>การลงทะเบียนทั้งหมดสำหรับอีเวนต์นี้</li>
            <li>ข้อมูลผู้เข้าร่วมอีเวนต์ทั้งหมด</li>
          </ul>
        </div>
