...
export default function DeleteStaffModal({ isOpen, onClose, staffId, staffName }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/data/staff/delete/${staffId}`, {
        method: "POST",
        headers: {
      ...
      setError(error.message || "An error occurred while deleting the staff member.");
      setIsDeleting(false);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 relative z-10">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-6">
          คุณแน่ใจหรือไม่ว่าต้องการลบสตาฟ {staffName ? <strong className="dark:text-white">{staffName}</strong> : "คนนี้"}?
        </p>
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 p-4 mb-6">
          ...
          <p className="text-red-700 dark:text-red-400 mt-2">
            การลบสตาฟจะเพิกถอนสิทธิ์การเข้าถึงบูธทั้งหมด
          </p>
        </div>
