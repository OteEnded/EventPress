...
async function getOrganizersByOwnerUserId(ownerUserId) {
    ...
    const organizerQueryResult = await dbConnection.select({organizer_id: organizers.organizer_id}).from(organizers).where(eq(organizers.owner, ownerUserId)).orderBy(desc(organizers.created_at));
    if (organizerQueryResult.length === 0) {
        return null;
    }
