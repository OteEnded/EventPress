...
export default async function OrganizerDashboardPage() {
	...
	const event_description =
		"พวกเราทีม VitaminCNC จะมาช่วยน้องๆเสริมภูมิคุ้มกัน ซ้อมทำข้อสอบก่อนลงสนาม!! จากรุ่นพี่ผู้มีประสบการณ์ผ่าน";
		...
		for (let event of organize.events) {
			result.push(
				...
				>
					...
					<Link
						href={`/organizer/${organize["organizer"]["organizer_id"]}/event/${event["event_id"]}`}
					>
			...
			result.push(
				<div key={`${organize["organizer"]["organizer_id"]}`}>
					...
					<div className="px-4 py-2 flex justify-between items-center">
						...
						<Link
							href={`/organizer/${organize["organizer"]["organizer_id"]}/event/create`}
						>
							<button className="px-3 py-3 mt-3 bg-blue-600 text-white rounded-lg text-lg font-semibold hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 transition">
								สร้างอีเวนต์
							</button>
			...
			<div className="min-h-screen flex flex-col bg-[#5E9BD6] text-gray-700 dark:bg-gray-900 px-6">
				...
				<div className="flex justify-end my-5 gap-6">
					<Link href={`/organizer/create`}>
						<button className="px-3 py-3 mt-1 bg-blue-600 text-white rounded-lg text-lg font-semibold hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 transition">
							ลงทะเบียนองค์กร
						</button>
					</Link>
				...
				<div className="bg-white dark:bg-gray-800 dark:text-white p-16  border-primary flex flex-col w-full ">
					<h1 className="text-3xl font-semibold mb-4">
						{" "}
						อีเวนต์ของฉัน{" "}
					</h1>
					<div>{get_organizes()}</div>
				</div>
